...
export class Marble {
  ...
  private _rollPhase: number = 0;
  private _wobble: number = 0;
  ...
  // Draw a rice cake (songpyeon) shape
  private _drawSongpyeon(ctx: CanvasRenderingContext2D, r: number) {
    const w = r * 2;
    const h = r * 1.4;
    ctx.beginPath();
    ctx.moveTo(0, -h * 0.50);
    ctx.quadraticCurveTo(w * 0.60, -h * 0.10, w * 0.55, h * 0.45);
    ctx.quadraticCurveTo(0, h * 0.65, -w * 0.55, h * 0.45);
    ctx.quadraticCurveTo(-w * 0.60, -h * 0.10, 0, -h * 0.50);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.ellipse(-r * 0.3, -r * 0.2, r * 0.35, r * 0.25, -0.2, 0, Math.PI * 2);
    ctx.fillStyle = 'white';
    ctx.fill();
    ctx.restore();
  }
  ...
  update(deltaTime: number) {
    ...
    this._rollPhase += speed * 6;
    const targetWobble = Math.min(1, speed * 8);
    this._wobble += (targetWobble - this._wobble) * 0.2;
  ...
  private _drawMarbleBody(ctx: CanvasRenderingContext2D, isMinimap: boolean) {
    ...
    ctx.rotate(this.angle + this._rollPhase * 0.20);
    const sx = 1 + this._wobble * 0.08;
    const sy = 1 - this._wobble * 0.08;
    ctx.scale(sx, sy);
    this._drawSongpyeon(ctx, this.size / 2);
    ctx.restore();
  }
